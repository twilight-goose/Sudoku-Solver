...
def main():
    ...
    grid = classes.Grid([[0] * 9] * 9, show_wrong=False, show_possible=False)
    ...
    solve_button = ui.Button("Solve", (525, 25))
    check_button = ui.Button("Check", (525, 100))
    show_button = ui.Button("Show Possible", (525, 175))
    clear_button = ui.Button("Clear", (525, 250))
    # list of buttons
    buttons = [solve_button, check_button, clear_button, show_button]
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    grid.set_square(selected_cord, number)      # place a number in a square
                elif event.key == pygame.K_F12:         # F12 loads the "hardest" grid
                    grid = classes.Grid(HARDEST_GRID)
